...
const ITEMS = {
  ...
  // ── Wave 3: New feature drop — /upgrade system ────────────
  // `forgehammer` is the consumable material /upgrade consumes alongside
  // coins to level up an equippable item. Buyable in the shop, or won
  // from /spin like any other gacha drop.
  forgehammer: { id: 'forgehammer', name: 'Forge Hammer', emoji: '🔨✨', slot: 'consumable', rarity: 'uncommon', price: 600, source: 'shop' },
  risingaegis: { id: 'risingaegis', name: 'Rising Aegis', emoji: '🛡️🌟', slot: 'armor', rarity: 'epic', price: 4500, source: 'shop' },
  stormfang: { id: 'stormfang', name: 'Stormfang Blade', emoji: '🌩️', slot: 'weapon', rarity: 'epic', source: 'gacha' },
};
...
// Base coin value for gacha-only items (no shop `price`) when computing
// /upgrade cost, keyed by rarity — mirrors roughly what an equivalent
// shop item of that rarity would cost.
const RARITY_BASE_VALUE = { common: 200, uncommon: 500, rare: 1200, epic: 3000, legendary: 8000 };

const UPGRADE_MAX_LEVEL = 5;

/** Coin value used as the basis for /upgrade cost math. */
function baseValue(item) {
  return item.price || RARITY_BASE_VALUE[item.rarity] || 300;
}

/**
 * Coins + Forge Hammers needed to go from `currentLevel` to `currentLevel + 1`
 * for the given item. Returns null if already at UPGRADE_MAX_LEVEL.
 */
function upgradeCost(item, currentLevel) {
  ...
  return {
    nextLevel,
    coins: Math.round(baseValue(item) * 0.5 * nextLevel),
    hammers: nextLevel,
  };
}

/**
 * Small combat stat bonus derived from a user's equipped weapon/armor and
 * their per-item /upgrade levels. Used by /fight (and any future combat
 * command) — purely additive flavor on top of existing damage rolls, never
 * touches who wins (that stays random/skill-based).
 */
function getCombatBonus(user) {
  const equipped = user.equipped || {};
  const upgrades = user.upgrades || {};
  const atkBonus = equipped.weapon ? (upgrades[equipped.weapon] || 0) * 2 : 0; // +2 dmg per weapon level
  const defBonus = equipped.armor ? (upgrades[equipped.armor] || 0) * 1 : 0; // -1 dmg taken per armor level
